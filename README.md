...
![Vibespace — a real shell running git log beside a browser pane, with the file tree and the view switcher above](./docs/vibespace-demo-still.png)

From a real session, not a mockup: that is a real pty running a real
`git log`, and the page on the right is genuinely loaded in the pane beside
it. Captured at full Retina resolution, so the text stays sharp on a laptop
display.

**[▶ Watch the tour (14s)](./docs/vibespace-demo.mp4)** — GitHub plays it
inline when you open the file. It moves through four screens:

- **Terminals** — a shell beside a browser pane: the two kinds of pane a
  workspace can hold, side by side
- **Skills** — the skills discovered from disk, ready to drag onto a
  running agent
- **Settings** — the theme list
- **Swarm** — the mission canvas

It ends where it started, back on the terminals.
